import os, jwt, aiosqlite, hashlib, hmac, time, smtplib
...
app = FastAPI(title="Seven 70/30 Backend")
...
async def register(inp: AuthIn):
    async with aiosqlite.connect(DB) as db:
        try:
            await db.execute("INSERT INTO users(email, passhash) VALUES (?,?)",
                             (inp.email.lower(), bcrypt.hash(inp.password)))
            await db.commit()
        except Exception as e:
            if "UNIQUE" in str(e): raise HTTPException(409, "Email already exists")
            raise
...
async def reset_request(email: EmailStr):
    ...
    async with aiosqlite.connect(DB) as db:
        await db.execute("INSERT OR REPLACE INTO resets(email, code, ts) VALUES (?,?,?)",
...
async def reset_verify(payload: ResetVerifyIn):
    async with aiosqlite.connect(DB) as db:
        ...
        await db.execute("UPDATE users SET passhash=? WHERE email=?",
...
def is_admin(email, password):
    return (email == os.getenv("ADMIN_EMAIL", "admin@example.com") and
...
def root():
 ...
 @app.get("/api/cpx/link") async def cpx_link(user=Depends(get_user)):     ext_user_id = user["email"]     digest = hmac.new(CPX_SECURE_KEY.encode(), ext_user_id.encode(), hashlib.sha256).hexdigest()     url = f"https://offers.cpx-research.com/index.php?app_id={CPX_APP_ID}^^&ext_user_id={ext_user_id}^^&secure_hash={digest}"     return {"url": url}
